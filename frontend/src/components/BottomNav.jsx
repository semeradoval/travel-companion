...
export default function BottomNav() {
  ...
  return (
    <nav style={{
      ...
      <button onClick={() => navigate('/search')} style={navItemStyle(isActive('/search'))}>
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={isActive('/search') ? 'white' : '#9ca3af'} strokeWidth="2" strokeLinecap="round">
          <circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/>
        </svg>
        <span style={{ fontSize: 11, color: isActive('/search') ? 'white' : '#9ca3af' }}>Hledat</span>
      </button>
