...
export default function EditTopic() {
  ...
  useEffect(() => {
    api.topic.get(id)
      .then(t => { setCategory(t.category); setTitle(t.title) })
      .catch(console.error)
  ...
  async function handleSubmit() {
    ...
    setSaving(true)
    try {
      await api.topic.update({ id, title: title.trim(), category })
      navigate(`/topic/${id}`)
    } catch (e) {
    ...
    <div style={wrap}>
      <div style={{ padding: '16px 20px', background: 'white', borderBottom: '1px solid #e5e7eb' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <h2 style={{ fontSize: 16, fontWeight: 700, color: '#111827', margin: 0 }}>Upravit téma</h2>
...
const inputStyle = (err) => ({ width: '100%', padding: '14px 16px', borderRadius: 16, border: `2px solid ${err ? '#dc2626' : '#111827'}`, fontSize: 16, color: '#111827', background: '#f9fafb', outline: 'none', boxSizing: 'border-box' })
