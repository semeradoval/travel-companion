...
export default function EntryDetail() {
  ...
  useEffect(() => {
    api.entry.get(entryId)
      .then(e => { setEntry(e); return api.topic.get(e.topicId) })
      .then(t => setTopic(t))
      .catch(console.error)
  ...
  async function handleDelete() {
    try {
      await api.entry.delete(entryId)
      navigate(`/topic/${id}`)
  ...
  return (
    <div style={wrap}>
      <div style={{ padding: '16px 20px', background: 'white', borderBottom: '1px solid #e5e7eb' }}>
        <button onClick={() => navigate(`/topic/${id}`)} style={backBtn}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginTop: 4 }}>
          <div style={{ flex: 1 }}>
            ...
            {!loading && <h2 style={{ fontSize: 20, fontWeight: 700, color: '#111827', margin: '8px 0 0', lineHeight: 1.3 }}>{entry?.title}</h2>}
          </div>
          <div style={{ display: 'flex', gap: 8, flexShrink: 0, marginTop: 4 }}>
            <button onClick={() => navigate(`/entry/${entryId}/edit`)} style={iconBtn('#f3f4f6')} aria-label="Upravit">
            ...
            <button onClick={() => setConfirm(true)} style={iconBtn('#fef2f2')} aria-label="Smazat">
      ...
      <div style={{ flex: 1, padding: 20, paddingBottom: 88 }}>
        {loading
          ? <div style={{ height: 120, background: '#f3f4f6', borderRadius: 16 }} />
          : <div style={{ background: 'white', borderRadius: 16, padding: 20, border: '1.5px solid #d1d5db', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
              <p style={{ fontSize: 16, color: '#1f2937', lineHeight: 1.65, whiteSpace: 'pre-wrap' }}>{entry?.text}</p>
            </div>
        }
      </div>

      <BottomNav />
      {confirm && <ConfirmDialog message={`Smazat zápisek „${entry?.title}"?`} onConfirm={handleDelete} onCancel={() => setConfirm(false)} />}
    </div>
  )
}
...
const backBtn = { display: 'flex', alignItems: 'center', gap: 4, fontSize: 14, fontWeight: 600, color: '#374151', background: 'none', border: 'none', cursor: 'pointer', padding: '4px 0', minHeight: 44, maxWidth: '100%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }
